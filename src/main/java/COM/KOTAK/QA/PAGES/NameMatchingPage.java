	...
	public void listNameMatchingRecord(String SheetName)
		...
		for (Map<String, String> map : testData) {
			...
			if (elementUtil.getText(validateFilterMsg).equals(ValidateFilterMsg)) {
				System.out.println(
						"No Records available for List operation Validation Message Displayed Is Correct for Product Type "
								+ ProductType + "");
				...
				if (elementUtil.getAttribute(productType).equals(ProductType)) {
					System.out.println("Name Matching Record " + ProductType + " is Displayed in List with Amount "
							+ AmountAbove + " and Percentage " + Percentage);
				} else {
					System.out.println("Name Matching Record " + ProductType + " is not Displayed in List");
				}
			}
...
//				String ProductType = map.get("Product Type");
		...
		String ValidationMsg = "Deleted successfully.";
		String CancelMsg = "db.nameMatch.cancel";
		...
		try {
			if (elementUtil.getText(errormessages).equals(CancelMsg)) {
				System.out.println("Cancel Button is working fine for Name Matching Delete Screen");
			} else {
				System.out.println("Cancel Button is not working for Name Matching Delete Screen");
			}
		} catch (Exception e) {
			System.out.println("Cancel Button is not working for Name Matching Delete Screen");
		}
		...
		try {
			if (elementUtil.getText(messages).equals(ValidationMsg)) {
				System.out.println("Name Matching Record deleted Successfully");
			} else {
				System.out.println("Name Matching Record is failed to delete");
			}
		} catch (Exception e) {
			System.out.println("Name Matching Record is failed to delete");
		}
...
//							System.out.println("Validation message is displayed properly for Invalid fields");
	...
	public void modifyNameMatchingRecord(String SheetName) throws InterruptedException, InvalidFormatException, IOException {
		...
		String ValidationMsg = "db.nameMatch.modify.success";
		String CancelMsg = "db.nameMatch.cancel";
		...
		try {
			if (elementUtil.getText(errormessages).equals(CancelMsg)) {
				System.out.println("Cancel Button is working fine for Name Matching Modify Screen");
			} else {
				System.out.println("Cancel Button is not working for Name Matching Modify Screen");
			}
		} catch (Exception e) {
			System.out.println("Cancel Button is not working for Name Matching Modify Screen");
		}
		...
		try {
			if (elementUtil.getText(messages).equals(ValidationMsg)) {
				System.out.println("Name Matching Record modified Successfully");
			} else {
				System.out.println("Name Matching Record is failed to modify");
			}
		} catch (Exception e) {
			System.out.println("Name Matching Record is failed to modify");
		}
